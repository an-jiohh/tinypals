---
name: tinypals-hatch-pet
description: Create, repair, validate, QA, export, and register TinyPals desktop pet hatch assets. Use when Codex needs to generate a new TinyPals app character, convert a hatch-pet atlas into src/renderer/assets asset-pack files, update TinyPals petAssetRegistry static imports, or validate TinyPals app pet spritesheets. This fork is self-contained and preserves the hatch-pet generation pipeline while adding TinyPals project output rules.
---
...
This skill is a self-contained fork of the original hatch-pet workflow. Keep the original generation, repair, validation, visual QA, and deterministic atlas scripts as the source of truth for character creation. Add the TinyPals-specific behavior only at the final export and app-registration step.
...
## TinyPals Project Overlay

- Use the original workflow below for character generation, repair, atlas composition, contact-sheet QA, and preview QA.
- Treat `final/spritesheet.webp` or another final transparent 8x9 hatch-pet atlas as the production source. Contact sheets are QA artifacts only and must not be used as production input.
- After final atlas validation passes, export a TinyPals asset pack with `scripts/export_tinypals_asset_pack.py`.
- TinyPals asset packs live under `src/renderer/assets/<asset-id>/` and contain `pet.json`, nine row PNG spritesheets, `spritesheet-2x.png`, and optionally `spritesheet.webp`.
- Registry updates are explicit code patches. Do not auto-generate `src/renderer/src/petAssetRegistry.ts`; instead add static imports and the file map using the existing registry pattern, then update registry tests.
- The TinyPals app contract uses these states: `idle`, `running-right`, `running-left`, `waving`, `jumping`, `failed`, `waiting`, `running`, and `review`.
...
```bash
SKILL_DIR="${CODEX_HOME:-$HOME/.codex}/skills/tinypals-hatch-pet"
...
## TinyPals Project Export Workflow

When the final transparent hatch-pet atlas is ready, export it into the TinyPals app asset-pack format:

```bash
SKILL_DIR="${CODEX_HOME:-$HOME/.codex}/skills/tinypals-hatch-pet"
python "$SKILL_DIR/scripts/export_tinypals_asset_pack.py" \
  --repo-root /absolute/path/to/repo \
  --asset-id "<kebab-id>" \
  --display-name "<Display Name>" \
  --description "<one sentence>" \
  --license custom \
  --source-atlas /absolute/path/to/final/spritesheet.webp
```

The exporter writes:

```text
src/renderer/assets/<asset-id>/
  pet.json
  spritesheet-2x.png
  <asset_id>_idle.png
  <asset_id>_running_right.png
  <asset_id>_running_left.png
  <asset_id>_waving.png
  <asset_id>_jumping.png
  <asset_id>_failed.png
  <asset_id>_waiting.png
  <asset_id>_running.png
  <asset_id>_review.png
```

If the source atlas is WebP, the exporter also copies it to `spritesheet.webp`. If the source atlas is PNG and a WebP copy is needed, pass `--write-webp`.

After export, register the asset pack manually:

1. Add static `pet.json` and row PNG imports in `src/renderer/src/petAssetRegistry.ts`, following the existing `dough-penguin` pattern.
2. Add the asset pack id to the registry map and option list.
3. Update `src/renderer/src/petAssetRegistry.test.ts` so the new pack validates and unknown ids still fall back to `dough-penguin`.
