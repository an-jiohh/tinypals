...
이 문서는 현재 코드 기준으로 TinyPals Desktop Pet의 기술 스택, 런타임 구조,
...
- `npm run skill:tinypals:install`, `npm run skill:tinypals:validate`: 선택적 asset 생성 skill 설치/검증
...
분리되어 있습니다.
asset pack 추가 기준은 `docs/asset-packs.md`에 분리되어 있습니다.
...
`CFBundleIconFile`을 `TinyPals` 기준으로 맞춥니다. 패키징 앱은 `package.json`의
...
수동 검증 시에는 `TINYPALS_USER_DATA_DIR` 환경 변수로 저장 위치를 바꿀 수 있습니다.

### Preload

preload는 renderer에 제한된 API만 노출합니다. renderer는 Node API나 Electron IPC에
직접 접근하지 않고 `window.tinyPalsDesktop`만 사용합니다.
...
`TinyPalsDesktopApi`는 renderer가 사용할 수 있는 공개 경계입니다.
...
같은 오른쪽 아래 기본 위치로 되돌립니다. 펫 창 기본 크기는 144x156이고, 사용자가
우하단 resize handle을 드래그하면 96:104 비율을 유지해 96x104부터 384x416까지
조절한 크기를 저장합니다.
...
24px margin의 오른쪽 아래에 배치하고 저장하되, 현재 또는 저장된 펫 창 크기는 유지합니다.
...
renderer는 `petAssetRegistry.ts`에 등록된 asset pack의 `pet.json`과 상태별 PNG row
spritesheet를 import한 뒤 `selectedAssetPack`과 `PetMood`에 맞는 asset metadata를
선택합니다. 설정 창의 `Character` 선택값은 `settings:changed` 이벤트로 펫 창에
전파되며, 알 수 없는 pack id는 기본 `dough-penguin`으로 fallback합니다. 상태별 asset
파일이 빠져 있으면 idle asset으로 fallback합니다.

기본 TinyPals asset pack은 hatch-pet의 2x 투명 atlas `spritesheet-2x.png`를 원본으로 둡니다.
renderer용 상태별 PNG row spritesheet는 `npm run assets:tinypals`로 atlas의 384x416 셀을
검증한 뒤 원본 해상도를 유지한 row strip으로 파생합니다. CSS는 이를 96x104 논리 frame
비율로 그려 Retina 환경과 창 확대에서 흐림을 줄입니다. `tinypals-test` pack은 선택 흐름
검증용 복사본입니다. QA contact sheet는 alpha가 사라진 검토용 이미지이므로 asset source로
사용하지 않습니다.

`skills/tinypals-hatch-pet`는 공개 레포에 포함된 선택적 개발 도구입니다. 앱 런타임에는
필요하지 않으며, 새 character atlas를 만들거나 기존 atlas를 TinyPals asset pack으로
export할 때만 사용합니다. 새 asset을 사용자 선택지로 노출하려면 자동 생성이 아니라
`petAssetRegistry.ts`에 명시적 static import를 추가합니다.
